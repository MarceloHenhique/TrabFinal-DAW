    ...
    angular.module("cfp.loadingBarInterceptor", [ "cfp.loadingBar" ]).config([ "$httpProvider", function(a) {
        ...
        this.$get = [ "$injector", "$document", "$timeout", "$rootScope", function(a, b, c, d) {
            function e() {
                k || (k = a.get("$animate"));
                var e = b.find(n).eq(0);
                c.cancel(m), r || (d.$broadcast("cfpLoadingBar:started"), r = !0, v && k.enter(o, e, angular.element(e[0].lastChild)),
                u && k.enter(q, e, angular.element(e[0].lastChild)), f(w));
            }
            function f(a) {
                if (r) {
                    var b = 100 * a + "%";
...
app.controller("questionController", [ "$scope", "$timeout", "questionService", function($scope, $timeout, questionService) {
    ...
    this.validateQuestion = function($ngscope, opt) {
        ...
        if (the_statement == null || the_statement.length == 0) return false;
        return the_statement;
    };
    this.answer = function() {
        var _selector = sprintf("[name=in-question-%d]:checked", this.current);
        var $child = $(_selector);
        if ($child.exists() == false) return alert("Please choose an option.");
        var _current = this.current;
        this.ans[_current] = $child.val();
        this.current = -1;
        $timeout(function() {
            self.current = _current + 1;
            if (self.current == self.list.length) self.processQuestions();
        }, 1e3);
    };
    this.processQuestions = function() {
        self.end = true;
        self.list.forEach(function(question, index) {
            if (question["answer"] == self.ans[index]) console.log("Acerto.");
        });
        var _Data = {
            user_id: 1,
            exam_id: 11,
        ...
        return {
            ...
            link: function(scope, elm, attrs, ctrl) {
                ctrl.$validators.cpfValidator = function(modelValue, viewValue) {
                    ...
                    if (viewValue === undefined || the_cpf.length != 11) return false;
                    var forValidate = the_cpf.substring(9, 11);
                    function calcDigit(_from) {
                        var sum = 0;
                        for (var fat = _from; fat >= 2; fat--) sum += parseInt(the_cpf[_from - fat]) * fat;
                        var digit = sum * 10 % 11;
                        if (digit == 10) digit = 0;
                        return digit;
                    }
                    if (calcDigit(10) != parseInt(forValidate[0]) || calcDigit(11) != parseInt(forValidate[1])) return false;
                    return true;
                };
            }
        };
...
app.controller('loginController', ['$scope', '$http', function($scope, $http, $user) {
    this.master = [];
    $scope.result = "Não logado" ;
    ...
    this.log = function() {
      ...
      $http.get("http://localhost/enem/user/'" + $scope.login + "'/'" + $scope.pssw + "'").success(function(response) {
        if(response == "true") {
          $scope.result = "Correct Login";
        } else $scope.result = "Incorrect Login";
      })
